...
static httpd_uri_t s_setup_handlers[] = {
    ...
    },
    {
        .uri = "/ota_status",
        .method = HTTP_GET,
        .handler = esp_br_web_ota_status_get_handler,
        .user_ctx = NULL,
    },
    {
        .uri = "/ota_upload",
        .method = HTTP_POST,
        .handler = esp_br_web_ota_upload_post_handler,
        .user_ctx = NULL,
    },
    {
        .uri = "/ota_url",
        .method = HTTP_POST,
        .handler = esp_br_web_ota_url_post_handler,
        .user_ctx = NULL,
    },
#if CONFIG_OPENTHREAD_BR_SOFTAP_SETUP
    {
        .uri = "/scan",
        .method = HTTP_GET,
        .handler = esp_br_wifi_config_scan_get_handler,
        .user_ctx = NULL,
    },
    {
        .uri = "/submit",
        .method = HTTP_POST,
        .handler = esp_br_wifi_config_submit_post_handler,
        .user_ctx = NULL,
    },
    {
        .uri = "/wifi_status",
        .method = HTTP_GET,
        .handler = esp_br_wifi_config_status_get_handler,
        .user_ctx = NULL,
    },
    {
        .uri = "/hotspot-detect.html",
        .method = HTTP_GET,
        .handler = esp_br_wifi_config_captive_portal_get_handler,
        .user_ctx = NULL,
    },
    {
        .uri = "/generate_204",
    ...
    },
    {
        ...
        .method = HTTP_GET,
        .handler = esp_br_wifi_config_icon_get_handler,
        .user_ctx = NULL,
    },
    {
...
static httpd_handle_t *start_esp_br_http_server(const char *base_path, const char *host_ip)
{
    ...
    config.max_uri_handlers =
        (sizeof(s_resource_handlers) + sizeof(s_web_gui_handlers) + sizeof(s_setup_handlers)) / sizeof(httpd_uri_t) + 2;
    ...
    // Firmware uploads and Wi-Fi scans block the handling socket for a while
    ...
    httpd_server_register_http_uri(&s_server, s_setup_handlers, sizeof(s_setup_handlers) / sizeof(httpd_uri_t));
...
static void handler_got_ip_event(void *arg, esp_event_base_t event_base, int32_t event_id, void *event_data)
{
    ...
    sprintf(s_server.ip, IPSTR, IP2STR(&event->ip_info.ip));
    ESP_LOGI(WEB_TAG, "Web server reachable at http://%s", s_server.ip);
}

void esp_br_web_start(char *base_path)
{
    // The server is started before any network interface is up, so that the Wi-Fi provisioning
