...
/* Show a banner while the device runs the setup access point. */
(function() {
if (location.pathname === '/wifi_configuration.html')
  return;
fetch('/wifi_status')
    .then(function(r) { return r.json(); })
    .then(function(data) {
      ...
      banner.className = 'disabled-banner';
      banner.innerHTML =
          'The device is not connected to Wi-Fi and runs the setup access point ' +
          escapeHtml(data.softap_ssid) +
          '. <a href="/wifi_configuration.html">Configure Wi-Fi</a> to reach it on your network.';
      container.insertBefore(banner, container.firstChild);
    })
    .catch(function() {});
})();
